<template>
	<section class="flex flex-col items-center" :class="props.class">
		<template v-if="feedData.length">
			<div v-for="post in feedData" :id="`post-${getPostId(post.uri)}`" :key="post.uri" class="w-full">
				<BSkyPost :post="post" :modal-content-handler="handleModalContent" :compact="props.compact" />
			</div>
			...
			<div v-if="props.loadMore && cursor" class="mt-8 mb-16">
				<button id="bsky-load-more" :class="props.loadMoreClass" @click="loadMore">Load More Posts</button>
			</div>
		</template>
...
import { BlueskyImageElement, Props, RootProps } from "./types";
...
const props = withDefaults(defineProps<Props>(), {
	...
	postUserLinkClass: "text-ellipsis overflow-hidden whitespace-nowrap hover:underline dark:text-white",
	...
	repostClass: "flex gap-1 items-center ml-10 text-slate-600 dark:text-slate-400",
	repostTextClass: "text-sm text-slate-500 font-semibold",
	compactToggleClass: "text-sm text-right",
	loadMoreClass: "bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded",
	loadingPostClass: "w-full flex gap-2 p-4 border-b border-slate-300 dark:border-slate-800 animate-pulse",
	loadingPostAvatarClass: "bg-slate-200 w-14 h-14 rounded-full dark:bg-slate-800",
	loadingPostHeadlineClass: "h-2 bg-slate-200 rounded col-span-2 dark:bg-slate-800",
	loadingPostLines: 3,
...
const handleModalContent = (e: Event, image: BlueskyImageElement): void => {
...
provide(
	...
	computed(() => {
		...
		const rootProps = keys.reduce<Partial<RootProps>>((acc, curr) => {
			// Use type narrowing to ensure compatibility
			const value = props[curr];
			if (typeof value === "string" || typeof value === "number" || value === undefined) {
				/** @ts-expect-error Work this out later */
				acc[curr] = value;
			}
			return acc;
		}, {}) as RootProps;
