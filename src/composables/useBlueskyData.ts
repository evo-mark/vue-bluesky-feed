...
export const useBlueskyData = (props: Pick<Props, "username" | "feed" | "search" | "limit" | "linkClass">) => {
	const isLoading = ref(false);
	const feedData: Ref<BlueskyPost[]> = ref([]);
	const cursor = ref("");
	const limit = computed(() => props.limit ?? 25);

	const loadFeed = (newData: BlueskyPost[]) => {
	...
	const updateFeed = async () => {
		...
		if (props.username) {
			...
			if (success) {
				...
				console.error(`[Vue Bluesky Feed] Error loading feed for username "${props.username}"`);
			}
			...
			if (success) {
				...
				console.error(`[Vue Bluesky Feed] Error loading feed for "${props.feed}"`);
			}
			isLoading.value = false;
			...
			if (success) {
				...
				console.error(`[Vue Bluesky Feed] Error loading feed for search term "${props.search}"`);
			}
			isLoading.value = false;
		} else {
			isLoading.value = false;
