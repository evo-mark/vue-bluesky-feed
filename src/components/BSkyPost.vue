<template>
    <article :class="rootProps.postClass">
        <p v-if="post.isRepost" :class="rootProps.repostClass">
            ...
            <span :class="rootProps.repostTextClass"
                >Reposted by {{ post.repostBy }}</span
            >
        </p>
        ...
        <div class="flex gap-2">
            <img
                ...
                :src="post.avatar"
                alt="profile picture"
                :class="rootProps.postAvatarClass"
            />
            <div class="w-full">
                <header :class="rootProps.postHeaderClass">
                    ...
                    >
                        <span
                            :class="rootProps.postUsernameClass"
                            v-text="post.username"
                        />
                        <span>&nbsp;</span>
                        <span
                            :class="rootProps.postUserHandleClass"
                            v-text="`@${post.handle}`"
                        />
                    </a>
                ...
                <p :class="rootProps.postTextContentClass">
                    <template v-for="textEl in post.text">
                        <span
                            v-if="textEl.setInnerHtml"
                            v-html="textEl.val"
                        ></span>
                ...
                <template v-if="post.images?.length">
                    <ToggleShow
                        v-if="compact"
                        :show="showImageGrid"
                        @toggle="showImageGrid = !showImageGrid"
                    >
                    ...
                    <Vue3SlideUpDown v-model="showImageGrid">
                        <div
                            :class="{
                                ...
                                ['mt-4 ' + rootProps.postImageGridClass]:
                        ...
                        >
                            <a
                                ...
                                :target="linkTarget"
                                :rel="
                                    linkTarget === '_blank' ? 'noopeener' : ''
                                "
                                @click="
                                    ($event) =>
                                        modalContentHandler($event, image)
                                "
                            >
                                <img
                                    :src="image.thumb"
                ...
                <template v-if="post.card">
                    <ToggleShow
                        v-if="compact"
                        :show="showPostCard"
                        @toggle="showPostCard = !showPostCard"
                    >
                        Card
                    </ToggleShow>
